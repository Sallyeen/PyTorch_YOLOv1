...
class myYOLO(nn.Module):
    def __init__(self, device, input_size=None, num_classes=20, trainable=False, 
        ...
        self.conf_thresh = conf_thresh                 # 置信度得分阈值
    ...
    def create_grid(self, input_size):
        ...
        grid_xy = torch.stack([grid_x, grid_y], dim=-1).float() # [hs, ws, 2]
    ...
    def nms(self, dets, scores):
        ...
        x2 = dets[:, 2]  #xmax
        y2 = dets[:, 3]  #ymax
        areas = (x2 - x1) * (y2 - y1) # 计算bbox面积
        order = scores.argsort()[::-1] # 返回scores从大到小排序后的指针        
        keep = []                                             
        while order.size > 0:
            i = order[0]
            keep.append(i)
            ...
            # 滤除超过nms阈值的检测框,输出iou在阈值之内的第零维坐标
    ...
    def postprocess(self, bboxes, scores):
        ...
        """
        cls_inds = np.argmax(scores, axis=1)  # 每个点最有可能的类别的指针
        scores = scores[(np.arange(scores.shape[0]), cls_inds)]   # 提取出【最有可能的类别的】scores值    
        # threshold
        keep = np.where(scores >= self.conf_thresh)  # 提取出scores大于指定阈值的指针
        bboxes = bboxes[keep] # 提取出【scores大于指定阈值的】bboxes值
        scores = scores[keep] # 提取出【scores大于指定阈值的】scores值
        cls_inds = cls_inds[keep] # 提取出【scores大于指定阈值的】cls_inds值
        # NMS
